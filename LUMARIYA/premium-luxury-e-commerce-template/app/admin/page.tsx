...
export default function AdminPage() {
    ...
    useEffect(() => {
        if (user && user.is_admin === 1) {
            ...
            fetchAllOrders()
                .then(orders => setRecentOrders(orders.slice(0, 4)))
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-border">
                                {recentOrders.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="py-12 text-center text-sm text-foreground/40">
                                            No orders yet
                                        </td>
                                    </tr>
                                    ...
                                    <tr key={order.id} className="hover:bg-foreground/[0.02] transition-colors">
                                        ...
                                        <td className="py-4 px-6 text-sm font-light">{order.customer_name || 'Unknown'}</td>
