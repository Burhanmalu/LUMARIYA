  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        {colors.map(({ name, hex, available }, index) => (
          <button
            key={`${name}-${index}`}
            onClick={() => handleSelect(name, available)}
            disabled={!available}
            className={`relative w-8 h-8 rounded-full transition-all duration-300 ${!available ? "opacity-30 cursor-not-allowed" : "hover:scale-110"
          ...
          >
            {selected === name && (
              <Check
                className={`absolute inset-0 m-auto w-4 h-4 ${hex === "#0A0A0A" ||
                    hex === "#000080" ||
                    hex === "#36454F" ||
                    hex === "#722F37" ||
                    hex === "#3E2723" ||
